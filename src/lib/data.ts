...
export function subscribeToRegistrations(db: Firestore, callback: (registrations: Registration[]) => void): Unsubscribe {
    if (!db) return () => { };
    const registrationsCol = collection(db, 'registrations');
    const q = query(registrationsCol, orderBy('createdAt', 'desc'));
...
export function subscribeToWinners(db: Firestore, callback: (winners: Winner[]) => void): Unsubscribe {
    if (!db) return () => { };
    const winnersCol = collection(db, 'winners');
    const q = query(winnersCol, orderBy('confirmedAt', 'desc'));
